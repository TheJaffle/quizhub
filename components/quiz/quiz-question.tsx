        ...
        {question.options.map((option, index) => {
          ...
          return (
            <motion.div key={option.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: index * 0.1 }}>
              <Button variant={getOptionVariant(option.id)} className={`w-full min-w-0 justify-start whitespace-normal text-left h-auto py-4 px-4 sm:px-6 ${isSelected ? "border-2 shadow-md" : ""} ${showCorrectIndicator ? "bg-green-50" : ""} ${showIncorrectIndicator ? "bg-red-50" : ""} hover:bg-accent/80 transition-all duration-200`} onClick={() => handleOptionSelect(option.id)} disabled={answered || isReviewMode}>
                <div className="flex min-w-0 items-center justify-between w-full gap-3 sm:gap-4">
                  <div className="flex min-w-0 items-center gap-3">
                    <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground font-medium">{option.label ?? option.id.toUpperCase()}</span>
                    <span className="min-w-0 break-words text-base leading-snug">{option.text}</span>
                  </div>
