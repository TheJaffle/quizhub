...
import { Award, BookOpen, Compass, DollarSign, FileQuestion, Home, LifeBuoy, Lightbulb, LogIn, LogOut, Medal, Newspaper, Swords, Trophy, UserPlus, Users, X } from "lucide-react";
  ...
  return (
    <aside className={cn("max-xl:fixed max-xl:top-0 max-xl:left-0 max-xl:h-full xl:sticky top-0 flex h-screen flex-col border-r bg-background transition-all duration-300", collapsed ? "w-[250px] xl:w-[70px]" : "w-[250px]", collapsed ? "max-xl:-translate-x-full" : "max-xl:translate-x-0")}>
      ...
      <div className="border-t py-4">
        <nav className="grid gap-1 px-2">
          {!isAuthLoading && currentUser ? <NavAction icon={<LogOut className="size-5" />} label="Déconnexion" onClick={handleLogout} /> : null}
          {!isAuthLoading && !currentUser ? (
            <>
              <NavItem href="/login" icon={<LogIn className="size-5" />} label="Connexion" active={isActive("/login")} />
              <NavItem href="/register" icon={<UserPlus className="size-5" />} label="Inscription" active={isActive("/register")} />
            </>
          ) : null}
        </nav>
      </div>
...
function NavAction({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  ...
  if (collapsed) {
    return (
      <div className="relative group flex justify-center items-center">
        <button type="button" onClick={handleClick} className="flex h-10 w-10 items-center justify-center rounded-md transition-colors hover:bg-accent hover:text-accent-foreground">
          {icon}
        </button>
      </div>
    );
  ...
  return (
    <button type="button" onClick={handleClick} className="flex items-center gap-3 rounded-md px-3 py-2 xl:py-2.5 text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground">
      <span>{icon}</span>
      <span>{label}</span>
    </button>
  );
